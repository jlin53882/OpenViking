...
| `limit` | No | Global maximum result count. Tool value wins; otherwise an explicitly configured `recallLimit` is used. If neither exists, the server default applies. |
| `scoreThreshold` | No | Minimum relevance score 0–1 sent to context search (defaults to plugin config). |
| `resourceTypes` | No | Context-search target types: `resource`, `user`, `agent`. Defaults to plugin `recallTargetTypes`. For an exact URI scope, use `ov_search`. |
...
| `targetUri` | `viking://~/memories` | Default exact search scope for `ov_search` and `memory_forget`; not used by `memory_recall`. |
...
| `recallLimit` | unset (server currently defaults to `10`) | Optional global result limit for auto-recall and `memory_recall`. If unset, both requests omit `limit` and `quotas`. |
