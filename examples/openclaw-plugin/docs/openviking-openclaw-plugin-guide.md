...
| `recallLimit` | 未配置（服务端当前默认 `10`） | auto-recall 与 `memory_recall` 的可选全局条数上限；仅显式配置时发送 |
...
| `POST /api/v1/search/search` | 带 session context、intent analysis 和 token budget 的检索 | 自动召回、`memory_recall` | 使用 `mode="context"`、`purpose="coding"`，可带 `session_id`、全局 `limit`、`max_tokens`；返回 `entries`、`digest`、`rendered`。未显式配置 recall limit 时不发送 `limit` 或 `quotas`，使用服务端默认 |
...
| `targetUri` | `viking://user/memories` | `ov_search` / `memory_forget` 默认精确搜索目标；`memory_recall` 不使用 |
...
| `recallLimit` | 未配置（服务端当前默认 `10`） | auto-recall 与 `memory_recall` 可选全局条数上限；只在显式配置时发送 |
...
| `targetUri` | `ov_search` / `memory_forget` 默认精确搜索范围 | `viking://user/memories` | 是 | 否 | — | `memory_recall` 已不接受该参数；精确 URI 范围检索请使用 `ov_search`。 |
...
| `recallLimit` | 自动召回 / `memory_recall` 全局返回上限 | 未配置（服务端当前默认 `10`） | 是 | 否 | — | 仅显式配置时原样发送 `limit`，不在客户端展开为 quotas；未配置时省略并使用服务端默认。 |
...
5. **服务端默认值兜底**：若以上均未设置 recall limit，请求省略 `limit` 和 `quotas`，由 OpenViking 服务端决定（当前默认 `10`）。
...
| 明明命中结果很多，但注入数量少 | 服务端 quota/全局 limit、`recallMaxInjectedChars`、`recallPreferAbstract` | 服务端条数或 token 预算太紧；必要时显式配置 `recallLimit` 或改成 abstract 优先。 |
