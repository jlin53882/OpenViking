...
| `candidateLimit` | 旧兼容字段，仍参与配置归一化，但不再影响 context-based recall |
...
| 字段 | 语义 | 范围/约束 | 默认来源 |
|---|---|---|---|
| `recallLimit` | auto-recall 与 `memory_recall` 的服务端全局返回上限；仅显式来源发送 | 1-50 | 未显式设置时省略，由服务端默认（当前 10） |
| `candidateLimit` | 旧兼容字段；不影响 context-based recall | 1-200 | 派生值 |
| `candidateMultiplier` | 旧兼容字段；不影响 context-based recall | 1-20 | 4 |
...
| `targetUri` | `ov_search` / `memory_forget` 的单一 URI 范围；不用于 `memory_recall` | 必须以 `viking://` 开头 | 无 |
...
| 工具链路测试 | session 配置影响后续 `memory_recall` 全局 limit 与 `ov_search` 的 limit/targetUri |
| 自动召回场景测试 | context engine assemble 阶段使用 session 有效配置 |
| 排序测试 | 旧 rankingWeights、categoryWeights、resourceTypeWeights 配置仍可归一化和持久化 |
