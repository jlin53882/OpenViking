...
| `limit` | number | 插件侧诊断值。auto-recall 与 `memory_recall` 仅在工具参数或运行期/静态 `recallLimit` 显式设置时发送全局 `limit`；否则使用服务端默认。 |
...
重点查看 `resourceTypes` 和 context search trace，确认服务端是否按请求类型检索 `user`、`agent`、`resource`。`memory_recall` 不接受精确 `targetUri`；该场景使用 `ov_search`。
