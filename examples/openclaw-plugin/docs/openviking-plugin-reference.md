...
| `targetUri` | string | `viking://user/memories` | — | `ov_search` / `memory_forget` 的默认精确搜索范围；`memory_recall` 使用 context search 的类型范围。 |
...
| `recallLimit` | number | 未配置（服务端当前默认 `10`） | — | auto-recall 与 `memory_recall` 的可选全局条数上限。仅显式配置时发送 `limit`；未配置时不发送 `limit` 或 `quotas`，使用服务端默认。 |
...
| `memory_recall` | `query`、`limit?`、`scoreThreshold?`、`resourceTypes?` | 通过 session-aware context/coding search 显式召回长期记忆或资源；精确 URI 范围搜索请用 `ov_search`，session 历史请用 archive 工具。 |
