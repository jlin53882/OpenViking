...
| is_active | bool | No | True | Initial Watch scheduling state. `false` is supported for Connector and native Feishu URL imports with an explicit `to` and `watch_interval > 0`. The paused Watch is created immediately, while the initial import still runs once |
...
- For Connector and native Feishu URL imports, `is_active=false` creates the Watch in a paused state before the initial import finishes. The initial import still runs once and its terminal task ID, status, and error are recorded on the Watch; periodic scheduling remains disabled.
