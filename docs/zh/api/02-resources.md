...
| is_active | bool | 否 | True | Watch 初始调度状态。设为 `false` 时，仅支持 Connector 或原生飞书 URL，并要求显式传入 `to` 和 `watch_interval > 0`。Watch 会立即以暂停状态创建，首次导入仍执行一次 |
...
- Connector 和原生飞书 URL 导入可设置 `is_active=false`，在首次导入完成前立即创建暂停状态的 Watch。首次导入仍执行一次，并将最终任务 ID、状态和错误记录到 Watch；周期调度保持关闭。
